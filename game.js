...
let player, obstacles, frames, score, gameOver, groundx, speed, gravity, difficultyCounter, animationId;
...
if (frames % 140 === 0) {
        ...
        let type = "single";
        if (typeChance < 0.1) type = "pterodactyl"; // ~1 з 10
        else if (typeChance < 0.4) type = "double";

        const type = Math.random() < 0.3 ? "double" : "single";
        obstacles.push({
            x: canvas.width,
            y: type === "pterodactyl" ? 150 : canvas.height - 64 - 100,
            width: 60,
            height: type === "pterodactyl" ? 64 : 100,
            type
        });
        difficultyCounter++;
    ...
    for (let i = 0; i < obstacles.length; i++) {
        obstacles[i].x -= speed;
        const img = obstacles[i].type === "double" ? treeDoubleImage : (obstacles[i].type === "pterodactyl" ? pterodactylImage : treeSingleImage);
...
music.pause();
music.currentTime = 0;
bangSound.play().catch(e => console.warn("Bang sound blocked:", e));
            totalPoints += score;
