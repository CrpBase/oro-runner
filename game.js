...
let player, obstacles, frames, score, gameOver, groundx, speed, gravity, difficultyCounter, animationId, skipFrameCooldown = 0;
...
function draw() {
    ...
    if (frames % 140 === 0 && skipFrameCooldown === 0) {
        const clusterChance = Math.random();
        if (clusterChance < 0.3 && speed <= 6) {
            const count = 2 + Math.floor(Math.random() * Math.min(2, 4 - speed));  // 2 або 3 пальми
...
window.backToMenu = window.backToMenu = function backToMenu() {
