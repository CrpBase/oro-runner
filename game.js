...
function resetGame() {
    player = { x: canvas.width * 0.06, y: canvas.height * 0.81, width: canvas.width * 0.12, height: canvas.height * 0.21, vy: 0, jumping: false };
    gravity = 0.60;
    obstacles = [];
    frames = 0;
    score = 0;
...
function draw() {
    ...
    if (frames % 140 === 0) {
        ...
        const height = canvas.height * 0.17;
        obstacles.push({
            x: canvas.width,
            y: canvas.height - 64 - height,
